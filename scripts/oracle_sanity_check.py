#!/usr/bin/env python3
"""
Oracle sanity check for GRAFT restricted-candidate-set delta rerank.

WHAT A PASS LOOKS LIKE:
  - All full-vocab math checks have KL < 1e-4 (should be ~0 up to float precision).
  - Restricted KL decreases monotonically as k increases toward vocab_size.
  - Delta magnitude is exactly 0.0 for all k (because ft = raw in oracle mode).
  - Top-1 agreement and top-5 overlap are 1.0 for all k.

WHAT A FAIL MEANS:
  - If full-vocab KL > 1e-4: the proxy tuning formula is implemented incorrectly.
  - If delta magnitude > 0: logit extraction or subtraction has a bug.
  - If restricted KL does not converge to 0 at k=vocab_size: renormalization bug.

This is a pure oracle test (large = raw = ft = same checkpoint). No cross-model
transfer is tested here. The purpose is to verify the math before plugging in a
