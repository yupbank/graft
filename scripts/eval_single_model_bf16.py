#!/usr/bin/env python3
"""
Standalone IFEval evaluation for a single model.

Usage:
  uv run python scripts/eval_single_model_bf16.py models/Qwen3-8B-Base-bf16
  uv run python scripts/eval_single_model_bf16.py models/Qwen3-14B-Instruct-bf16

Generates greedy responses for all 541 IFEval prompts, scores them against
IFEval constraints, and saves results.
"""
...
# ---------------------------------------------------------------------------
# IFEval checkers (same as eval_ifeval_score_bf16.py)
# ---------------------------------------------------------------------------
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python eval_single_model_bf16.py <model_path>")
        sys.exit(1)
    ...
    output_gen = str(PROJECT_DIR / "results" / f"generation_{model_name}.json")
    output_scores = str(PROJECT_DIR / "results" / f"ifeval_scores_{model_name}.json")
    ...
    for i, prompt_text in enumerate(prompts):
        ...
        t0 = time.time()
        tokens = generate_greedy(model, prompt_tokens, MAX_TOKENS, eos_id)
        gen_time = time.time() - t0
        text = tokenizer.decode(tokens)

        gen_results.append({
            "prompt_idx": i, "prompt": prompt_text,
            "response": text, "num_tokens": len(tokens), "time": gen_time,
        })

        if (i + 1) % 50 == 0 or i == len(prompts) - 1:
            elapsed = time.time() - total_t0
            rate = (i + 1) / elapsed * 60
            print(f"  [{i + 1}/{len(prompts)}] {rate:.1f} prompts/min, {elapsed:.0f}s elapsed")
            sys.stdout.flush()
    ...
    for i, gen in enumerate(gen_results):
        ...
        ev = evaluate_response(gen["response"], inst_ids, kwargs_list)
        scores.append({
    ...
    print()
    print("=" * 60)
    print(f"IFEVAL SCORES — {model_name}")
    print("=" * 60)
